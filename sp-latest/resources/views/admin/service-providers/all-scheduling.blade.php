                            ...
                            <div class="table-responsive mt-4">
                                <table class="table align-middle mb-0 table-hover" id="Transaction-History">
                                    ...
                                    <tbody>
                                        ...
                                        @if(count($fetch_all_scheduling) > 0)
                                            @foreach ($fetch_all_scheduling as $list_service_providers_ind)
                                                <tr>
                                                    <td>{{ $sr_no++ }}</td>
                                                    
                                                    <td>{{ $list_service_providers_ind->name_of_client }}</td>
                                                    
                                                    <td>{{ $list_service_providers_ind->agreement_status }}</td>
                                                    <td>
                                                        <a href="{{route('admin.view_update_service_provider_scheduling')}}/{{$list_service_providers_ind->id}}" class="btn btn-xs btn-warning" title="Edit"><i class="bx bx-pencil" aria-hidden="true"></i></a>
                                                        ...
                                                        @else
                                                            <a href="{{route('admin.assign_and_approve_service_provider')}}/{{$list_service_providers_ind->id}}" class="btn btn-xs btn-success" title="Activate Service Provider"><i class="bx bx-like" aria-hidden="true"></i></a>
                                                        @endif
                                                    </td>
                                                </tr>  
                                            @endforeach
