                    ...
                    <div class="card-body">
                        <div class="custom-align">
                            ...
                            <button class="btn btn-xs btn-primary ml-auto"
                                    onclick="window.location.href='{{ route('admin.view_add_service_provider') }}'">
                                Add
                            </button>
                        </div>
                                ...
                                <tbody>
                                    ...
                                    @if(!empty($providers))
                                        @foreach($providers as $provider)
                                            <tr>
                                                <td>{{ $sr_no++ }}</td>
                                                <td>{{ $provider->name ?? $provider->company_name }}</td>
                                                <td>{{ \Carbon\Carbon::parse($provider->created_at)->format('d M Y') }}</td>
                                                <td>
                                                    <!-- View Button -->
                                                    <a href="{{ route('admin.billing.edit', $provider->id) }}"
                                                       class="btn btn-xs btn-info" title="View">
                                                        <i class="bx bx-show"></i>
                                                    </a>

                                                    <!-- Generate Button (e.g., PDF, Report, etc.) -->
                                                    <a href="{{ route('admin.billing.save', $provider->id) }}"
