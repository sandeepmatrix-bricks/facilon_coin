...
$client = new Client();
        ...
        $data1 = array('key_person_name'=>'Deven Punse','key_person_email'=>'deven@matrixbricks.com');
        ...
        $response1 = $client->post('https://graph.microsoft.com/v1.0/users/no-reply@facilonservices.com/sendMail', [
                        ...
                        'json' => [
                            'message' => [
                                'subject' => 'Facilon Services - service provider status',
                                'body' => [
                                    'contentType' => 'HTML', 
                                    'content' => $emailContent2, 
                                ],
                                'toRecipients' => [
                                    ['emailAddress' => ['address' => 'deven@matrixbricks.com']],
                                ],
                            ],
                        ],
                    ]);
    ...
    Log::info('Email sent successfully to'.$response1->getStatusCode().' : ' . 'deven@matrixbricks.com');
} else {
    Log::error('Failed to send email. Status: ' . $response1->getStatusCode());
}

exit;
        $full_name = 'Facilon Matrix Broker2';
        $investor_password_main = 'Provider@123';
        $email_id = 'deven@matrixbricks.com';
        $full_name = $email_id;
...
// Get the inviteRedeemUrl
$inviteRedeemUrl = $data11['inviteRedeemUrl'] ?? null;

// Example: display or log it
if ($inviteRedeemUrl) {
    echo "Invite Redeem URL: " . $inviteRedeemUrl;
} else {
    echo "Invite redeem URL not found in response.";
}
...
dd($response11->json());
        ...
        $fetch_provider_details = DB::table('service_providers_scheduling')
          ...
          $service_provider_pan_no = $fetch_provider_details->pan_no;
          
          $service_provider_gst_no = $fetch_provider_details->gst_no;
          $service_provider_website = $fetch_provider_details->website;
          // $service_provider_rbi_regulation_name = $fetch_provider_details->rbi_regulation_name;
          // $service_provider_rbi_registration_no = $fetch_provider_details->rbi_registration_no;
          // $service_provider_name_of_sebi_regulation = $fetch_provider_details->name_of_sebi_regulation;
          $service_provider_sebi_registration_no = $fetch_provider_details->sebi_registration_no;
          // $service_provider_exchange_name = $fetch_provider_details->exchange_name;
          // $service_provider_segment_name = $fetch_provider_details->segment_name;
          $service_provider_bse_sebi_registration_no = $fetch_provider_details->bse_sebi_reg_no;
          $service_provider_bse_f_o_sebi_registration_no = $fetch_provider_details->bse_f_and_o_sebi_reg_no;
          $service_provider_date_of_agreement = $fetch_provider_details->date_of_agreement;
          $service_provider_place_of_execution = $fetch_provider_details->place_of_execution;
          $service_provider_reg_office_address = $fetch_provider_details->client_registered_office_address;
          $sp_primary_contact_name = $fetch_provider_details->primary_contact_name1;
          $sp_primary_contact_phone = $fetch_provider_details->primary_contact_phone1;
          $sp_primary_contact_email = $fetch_provider_details->primary_contact_email1;
          $sp_primary_contact_designation = $fetch_provider_details->primary_contact_designation1;
          $service_provider_type = $fetch_provider_details->type_of_entity;

          $sp_escalation_contact_name = $fetch_provider_details->escalation_contact_name1;
          $sp_escalation_contact_phone = $fetch_provider_details->escalation_contact_phone1;
          $sp_escalation_contact_email = $fetch_provider_details->escalation_contact_email1;

          if($service_provider_type == 1)
          {
            $customertypecode = 4;
          }elseif($service_provider_type == 3)
          {
            $customertypecode = 6;
          }elseif($service_provider_type == 2)
          {
            $customertypecode = 2;
          }else{
            $customertypecode = 1; 
    ...
    'scope' => $scope
        ...
        curl_setopt_array($curl2, array(
          ...
          CURLOPT_POSTFIELDS =>json_encode(array(
            ...
            'ss_pannumber'=>$service_provider_pan_no,
            'ss_cinnumber'=>$service_provider_cin_no,
            'ss_gstnumber'=>$service_provider_gst_no,
            'ss_sebiregistrationno'=>$service_provider_sebi_registration_no,
            'ss_bsesebiregnno'=>$service_provider_bse_sebi_registration_no,
            'ss_bsefosebiregnno'=>$service_provider_bse_f_o_sebi_registration_no,
            'websiteurl'=>$service_provider_website,
            'ss_dateofagreement'=>$service_provider_date_of_agreement,
            'ss_placeofincorporation'=>$service_provider_place_of_execution,
            'address1_composite'=>$service_provider_reg_office_address,
...
curl_setopt_array($curl5, array(
  ...
  CURLOPT_CUSTOMREQUEST => 'POST',
  CURLOPT_POSTFIELDS =>json_encode(array(
                'firstname' => $sp_primary_contact_name,
                'emailaddress1'=>$sp_primary_contact_email,
                'telephone1'=>$sp_primary_contact_phone,
                'accountrolecode'=>1
  ...
  CURLOPT_HTTPHEADER => array(
    ...
    'Content-Type: application/json',
    'Authorization: Bearer '.$accessToken,
    'Cookie: ARRAffinity=5230b2973292d874dbd60cb18b78265e56d04676774b56bbd5ba0733528e55cbeb29bb4b19df1459d2ea781b605f33064b9fea1f144b525b9d7676625c4ffb5408DD8C9BFFD43A380000001099362572; ReqClientId=188686af-4586-4c9f-a53c-13cbdf72ee35; last_commit_time=2025-05-06 12:26:33Z; orgId=e4045f65-e6f7-ee11-904b-6045bde84adc'
  ),
));
...
$primary_contact_id = $data5["contactid"];

$curl6 = curl_init();

curl_setopt_array($curl6, array(
  CURLOPT_URL => 'https://fportalvapt.crm8.dynamics.com/api/data/v9.2/accounts('.$account_id.')',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
  ...
  CURLOPT_CUSTOMREQUEST => 'PATCH',
  CURLOPT_POSTFIELDS =>'{
    "PrimaryContactId@odata.bind": "/contacts('.$primary_contact_id.')"
}',
  CURLOPT_HTTPHEADER => array(
    'Content-Type: application/json',
    'Authorization: Bearer '.$accessToken,
    'Cookie: ARRAffinity=5230b2973292d874dbd60cb18b78265e56d04676774b56bbd5ba0733528e55cbeb29bb4b19df1459d2ea781b605f33064b9fea1f144b525b9d7676625c4ffb5408DD8CA56B3775580000001099362572; ReqClientId=188686af-4586-4c9f-a53c-13cbdf72ee35; last_commit_time=2025-05-06 13:48:39Z; orgId=e4045f65-e6f7-ee11-904b-6045bde84adc'
  ),
));
...
curl_setopt_array($curl7, array(
  CURLOPT_URL => 'https://fportalvapt.crm8.dynamics.com/api/data/v9.2/contacts('.$primary_contact_id.')',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
  ...
  CURLOPT_CUSTOMREQUEST => 'PATCH',
  CURLOPT_POSTFIELDS =>'{
    "AccountId@odata.bind": "/accounts('.$account_id.')"
}',
  CURLOPT_HTTPHEADER => array(
    'Content-Type: application/json',
    'Authorization: Bearer '.$accessToken,
...
curl_setopt_array($curl8, array(
  ...
  CURLOPT_CUSTOMREQUEST => 'POST',
  CURLOPT_POSTFIELDS =>json_encode(array(
                'firstname' => $sp_escalation_contact_name,
                'emailaddress1'=>$sp_escalation_contact_email,
                'telephone1'=>$sp_escalation_contact_phone,
                'accountrolecode'=>2
...
$primary_contact_id8 = $data8["contactid"];
...
curl_setopt_array($curl9, array(
  ...
  CURLOPT_CUSTOMREQUEST => 'PATCH',
  CURLOPT_POSTFIELDS =>'{
    "PrimaryContactId@odata.bind": "/contacts('.$primary_contact_id8.')"
}',
  CURLOPT_HTTPHEADER => array(
    'Content-Type: application/json',
    'Authorization: Bearer '.$accessToken,
...
curl_setopt_array($curl10, array(
  CURLOPT_URL => 'https://fportalvapt.crm8.dynamics.com/api/data/v9.2/contacts('.$primary_contact_id8.')',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
